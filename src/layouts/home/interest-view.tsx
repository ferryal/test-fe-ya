...
import { IUser } from './profile-view';
...
export default function InterestView() {
  ...
  const getUserProfile = useCallback(async () => {
    ...
    try {
      const { data, status } = await axios.get(`${process.env.NEXT_BASE_API_URL}/getProfile`, {
        headers: {
    ...
    if (!selectedOption.length) {
      ...
      try {
        const { data, status } = await axios.put(
          `${process.env.NEXT_BASE_API_URL}/updateProfile`,
          {
            ...user,
            interests: formValue,
          },
  ...
  return (
    <div className="flex flex-col gap-8 md:gap-12 rounded-lg">
      {loading && !user ? (
        dashboardSkeleton
      ) : (
        <>
